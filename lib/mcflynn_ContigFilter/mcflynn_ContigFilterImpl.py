...
This sample module contains one small method that filters contigs.
    ...
    def run_mcflynn_ContigFilter(self, ctx, params):
        """
        ...
        #BEGIN run_mcflynn_ContigFilter

        # Print statements to stdout/stderr are captured and available as the App log
        logging.info('Starting run_mcflynn_ContigFilter function. Params=' + pformat(params))
        ...
        #END run_mcflynn_ContigFilter

        # At some point might do deeper type checking...
        if not isinstance(output, dict):
            raise ValueError('Method run_mcflynn_ContigFilter return value ' +
                             'output is not type dict as required.')
        # return the results
        return [output]

    def run_mcflynn_ContigFilter_max(self, ctx, params):
        """
        :param params: instance of mapping from String to unspecified object
        :returns: instance of type "ReportResults" -> structure: parameter
           "report_name" of String, parameter "report_ref" of String
        ...
        # ctx is the context object
        # return variables are: output
        #BEGIN run_mcflynn_ContigFilter_max
        ...
        try:
            ...
            raise ValueError(f'Cannot parse integer from min_length parameter {str(min_length_orig)}')
        if min_length < 0:
            raise ValueError(f'min_length parameter cannot be negative {str(min_length)}')

        try:
            max_length = int(max_length_orig)
        except ValueError:
            raise ValueError(f'Cannot parse integer from max_length parameter {str(max_length_orig)}')
        if max_length < 0:
            raise ValueError(f'max_length parameter cannot be negative {str(max_length)}')
        if max_length < min_length:
            raise ValueError(f'max_length parameter {str(max_length)} must be greater than min_length {str(min_length)}')
        if max_length > 9999999:
            raise ValueError(f'max_length parameter must be less than 9999999')
        ...
        #END run_mcflynn_ContigFilter_max

        # At some point might do deeper type checking...
        if not isinstance(output, dict):
            raise ValueError('Method run_mcflynn_ContigFilter_max return value ' +
                             'output is not type dict as required.')
        # return the results
        return [output]
    def status(self, ctx):
